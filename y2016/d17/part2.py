...
def solve(passcode):
    start = (0, 0, passcode)
    max_dist = _bfs(start=start, goal_func=_goal, neighbors_func=_valid_neighbors)
    answer = max_dist - len(passcode)
    return answer
...
def _test():
    ...
    for i, (pscd, expected) in enumerate(cases):
        actual = solve(pscd)
...
if __name__ == '__main__':
    _test()
    print('Part 2 solution is:', solve('qljzarfv'))
