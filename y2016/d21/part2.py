...
def _rotate_pos(x, s):
    # Just try all rotations until the end result is the given s
    num = 1
    while True:
        rotated = _rotate_left(num, s)
        attempt = _original_rotate_pos(x, rotated)
        if attempt == s:
            return rotated
        num += 1
...
def solve(s, fname):
    lines_rev = open(fname).readlines()[::-1]
...
if __name__ == '__main__':
    assert 'abcde' == solve('decab', 'test_data.txt')
    print()
    print('\nPart 2 solution is:', solve('fbgdceah', 'input.txt'))
