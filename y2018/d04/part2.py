...
def _find_winner(minutes):
    options = []
    for guard_id in minutes:
        for m, times in minutes[guard_id].items():
            options.append((times, m, guard_id))
    winner = max(options, key=lambda x: x[0])
    answer = winner[1] * winner[2]
    return answer
...
def main(fname):
    ...
    answer = _find_winner(minutes)
    return answer


if __name__ == '__main__':
    assert 4455 == main('./test.txt')
